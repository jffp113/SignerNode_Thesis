...
#BYZANTINE=(146.59.229.32 139.99.236.2)
...
#BYZANTINE=(146.59.229.32 139.99.236.2 51.79.53.170)
...
#BYZANTINE=(146.59.229.32 141.95.18.234 51.79.53.77 139.99.236.2)
...
#BYZANTINE=(146.59.229.32 141.95.18.234 51.79.53.77 139.99.236.2 139.99.171.133)
...
N=4
T=3
SCHEME=TRSA3072Optimistic
FILE="peerPermissionless.yaml"
...
do
  CMD="bash -c '
    ...
    docker-compose -p ${ID} -f ${DOCKER_COMPOSE_PATH}/${FILE} up --detach
    '
  "
...
do
  ...
  "
  (echo $CMD | ssh -t ubuntu@${s} bash)
  echo "byzantine \"${s}:${API_PORT}\"" >> peers.txt
  ID=$((ID + 1))
  PORT=$((PORT + 1))
  API_PORT=$((API_PORT + 1))
  CRYPTO_PORT=$((CRYPTO_PORT + 1))
