...
func (s *signerNodeClient) SendSignRequest(toSignBytes []byte, smartcontract string) (pb.ClientSignResponse,error) {
	switch s.protocol {
		case PermissionedProtocol: return s.permissionedProtocol(toSignBytes,smartcontract)
		case PermissionlessProtocol: return s.permissionlessProtocol(toSignBytes)
	}
...
func (s *signerNodeClient) permissionlessProtocol(toSignBytes []byte) (pb.ClientSignResponse,error) {
	panic("implement me")
}
