...
func TestRegisterHandler(t *testing.T) {
	...
	ic.RegisterHandler(SignClientRequest, func(content ICMessage, ctx P2pContext) HandlerResponse {
...
func TestEmitEvent(t *testing.T) {
	...
	ic.RegisterHandler(SignClientRequest, func(content ICMessage, ctx P2pContext) HandlerResponse {
		assert.Equal(t, []byte("Jorge: "), content.GetData())
		return CreateOkMessage(append(content.GetData(), []byte("Hello")...))
	})

	ic.RegisterHandler(SignClientRequest, func(content ICMessage, ctx P2pContext) HandlerResponse {
		return CreateOkMessage(append(content.GetData(), []byte(" World")...))
	})

	resp := ic.EmitEvent(SignClientRequest, &internalMessage{
		from:    "",
		to:      "",
		content: []byte("Jorge: "),
	})
